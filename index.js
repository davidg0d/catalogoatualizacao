...
async function comparePasswords(supplied, stored) {
  ...
  try {
    ...
    console.error("Erro na compara\xE7\xE3o de senhas:", error);
    return false;
  }
}
function setupAuth(app2) {
  const isProduction = process.env.NODE_ENV === "production";
  console.log(`Ambiente: ${isProduction ? "Produ\xE7\xE3o" : "Desenvolvimento"}`);
  const cookieSettings = {
    maxAge: 30 * 24 * 60 * 60 * 1e3,
    // 30 dias
    httpOnly: true,
    // No Replit, nÃ£o usamos secure=true para evitar problemas com proxy
    secure: false,
    // Usar 'lax' Ã© mais compatÃ­vel com o ambiente Replit
    sameSite: "lax",
    path: "/"
  };
  console.log("Configura\xE7\xE3o de cookies para sess\xE3o:", cookieSettings);
  const sessionSettings = {
    ...
    // Estamos usando o nome padrÃ£o 'connect.sid' para compatibilidade com o cÃ³digo existente
  };
    ...
    try {
      ...
      if (role === "customer") {
        ...
        const shopOwnerData = {
          ...
          subscriptionExpiresAt: new Date(Date.now() + 15 * 24 * 60 * 60 * 1e3)
          // 15 dias de trial
        };
  ...
  app2.post("/api/login", (req, res, next) => {
    console.log(`Tentativa de login para usu\xE1rio: ${req.body.username}`);
    passport.authenticate("local", (err, user) => {
      ...
      if (!user) {
        console.log(`Login falhou - usu\xE1rio ${req.body.username} n\xE3o encontrado ou senha incorreta`);
        return res.status(401).json({ error: "Credenciais inv\xE1lidas" });
      }
      req.login(user, (err2) => {
        if (err2) {
          console.error("Erro ao iniciar sess\xE3o:", err2);
          return next(err2);
        }
        console.log(`Login bem-sucedido para ${user.username} (${user.role})`);
        console.log("ID da sess\xE3o:", req.sessionID);
  ...
  app2.get("/api/user", (req, res) => {
    console.log("GET /api/user - Verificando autentica\xE7\xE3o");
    console.log("Sess\xE3o ID:", req.sessionID);
    ...
    console.log(`Usu\xE1rio autenticado: ${req.user.username} (${req.user.role})`);
    const { password, ...userWithoutPassword } = req.user;
    res.json(userWithoutPassword);
  });
  const isAuthenticated = (req, res, next) => {
    if (!req.isAuthenticated()) {
      return res.status(401).json({ error: "Acesso n\xE3o autorizado - Login necess\xE1rio" });
    }
    next();
  };
  ...
  const isCustomer = (req, res, next) => {
    if (!req.isAuthenticated() || req.user.role !== "customer" && req.user.role !== "shopowner" && req.user.role !== "admin") {
  ...
  const hasActiveSubscription = async (req, res, next) => {
    ...
    if (!shopOwner) {
      return res.status(403).json({ error: "Perfil de lojista n\xE3o encontrado" });
    }
    return next();
    next();
  };
...
var ensureDirectoryExists = (path4, description) => {
  if (!fs2.existsSync(path4)) {
    console.warn(`Aviso: O diret\uFFFDrio ${description} (${path4}) n\uFFFDo existe. Criando-o agora.`);
    fs2.mkdirSync(path4, { recursive: true });
  }
};
